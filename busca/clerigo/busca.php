...
$qr="SELECT * from pae_clerigo WHERE nome like '%$dados$busca%' order by id desc";
...
while ($dados = mysqli_fetch_array($limite)) { 
	...
	//DESCOBRIR A NOMEAÇÃO MAS ATUAL
	$qr_nomeacao = "SELECT * from pae_nomeacao where pessoa_id = '$codigo' order by data_nomeacao desc limit 0,1";
	$todos_nomeacao = mysqli_query($conn,"$qr_nomeacao"); 
	$tr_nomeacao = mysqli_num_rows($todos_nomeacao); // verifica o n&uacute;mero total de registros 
	while ($dados_nomeacao = mysqli_fetch_array($todos_nomeacao)) { 
	$data_nomeacao      = $dados_nomeacao["data_nomeacao"];
	$instituicao_id     = $dados_nomeacao["instituicao_id"];
	$instituicao        = $dados_nomeacao["instituicao"];
	$funcao_ministerial = $dados_nomeacao["funcao_ministerial"];
	}	
	...
	$dia = date('d', strtotime($data_nomeacao));
	$mes = date('m', strtotime($data_nomeacao));
	$ano = date('Y', strtotime($data_nomeacao));
	...
	$data_nomeacao_texto = "$dia de $mes_texto de $ano";
	
	//NOMEACOES
	if ($tr_nomeacao == "") {  $nomeacao_texto = ""; } else { $nomeacao_texto = "<br>&Uacute;ltima Nomea&ccedil;&atilde;o: $instituicao | Fun&ccedil;&atilde;o: $funcao_ministerial<br>Data da nomea&ccedil;&atilde;o: $data_nomeacao_texto"; }
...
echo"<div class='panel panel-default $panel_horizontal' style='background-color:#114a66;border:3px solid #114a66;border-radius:20px;'>
		...
		<div class='panel-body' style='background-color:#fff;'>
			...
			<div style='vertical-align: middle;' align='left' >
				...
				<font color='#666666'><b>CL&Eacute;RIGO: $nome</b>
				$nomeacao_texto
				</p>
