    ...
    <div class="panel-body">
        ...
        <tbody>
                        ...
                        $qr_mat = "SELECT DISTINCT nome,id,distrito,regiao,distrito_id
						            ,b.recebidos,b.excluidos
						 from pae_igreja a
  							JOIN (SELECT igreja_id, 
							        ...
							        FROM pae_membro GROUP BY igreja_id) b ON a.id=b.igreja_id
						 WHERE distrito_id = '$distrito' $ordem_texto";
						}
						else {
						 $qr_mat = "SELECT DISTINCT a.nome,a.id,a.distrito,a.regiao,a.regiao_id,a.distrito_id
						            ,b.recebidos,b.excluidos
						            FROM pae_igreja a
									JOIN (SELECT igreja_id, 
									SUM(CASE WHEN (ano_recepcao <= '$ano') then 1 ELSE 0 END) AS recebidos,
							        SUM(CASE WHEN (ano_exclusao BETWEEN '1' AND '$ano') then 1 ELSE 0 END) AS excluidos
							        FROM pae_membro GROUP BY igreja_id) b ON a.id=b.igreja_id
						 WHERE regiao_id = '$regiao' $ordem_texto";	
						}
						...
						$regiao_nome    = $dados_mat["regiao"];
							$recebidos  = $dados_mat["recebidos"];
							$excluidos   = $dados_mat["excluidos"];	
...
//							$excluidos   = $dados_idade["excluidos"];	
							...
							$mostrahora = date("h:i:sa");
//                        <td style='font-size:13px;color:#333333;text-align:center'>$mostrahora</td>
